...
describe('handlers champions league tests - neggative', () => {
  it('should handle an error while execution insert queries ', async () => {
    champLeague.insertChampLeague = jest
      .fn()
      .mockImplementationOnce(() => Promise.reject(new Error('Failed to execute query')));
    const result = await handlersChampLeague.addChampLeague();
    expect(result).toEqual({ message: 'Bad request', status: 400 });
  });

  it('should handle an error while execution select queries ', async () => {
    champLeague.selectFromChampLeague = jest
      .fn()
      .mockImplementationOnce(() => Promise.reject(new Error('Failed to execute query')));
    const result = await handlersChampLeague.geChampLeague();
    expect(result).toEqual({ message: 'Bad request', status: 400 });
  ...
  it('should handle an error while execution update queries ', async () => {
    champLeague.updateChampLeague = jest
      .fn()
      .mockImplementationOnce(() => Promise.reject(new Error('Failed to execute query')));
    const result = await handlersChampLeague.updateChampLeague();
    expect(result).toEqual({ message: 'Bad request', status: 400 });
  });

  it('should handle an error while execution delete queries ', async () => {
    champLeague.deleteChampLeague = jest
      .fn()
      .mockImplementationOnce(() => Promise.reject(new Error('Failed to execute query')));
    const result = await handlersChampLeague.deleteChampLeague();
    expect(result).toEqual({ message: 'Bad request', status: 400 });
  });
});
